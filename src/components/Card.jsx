...
import {
  ...
  handleFriendRequest,
  sendFriendRequest,
} from "../Collections/friendrequest.collection";
...
const Card = ({ img, name, userid, state, requestId, t, friendId, userId }) => {
  const { friends, userData, requestPending } = useStore((state) => state);

  // **Check if the user is already a friend**
  const isFriend = friends.some((friend) => friend.id === userid);
  const isPending = requestPending.some(
    (data) => data.friendId == userid || data.userId == userid
  );
      ...
      {state === "pendingFriends" ? (
        t === "sent" ? (
          ...
          <div className="pendingFriendBtn">
            <button
              className="btn-btn btn-accept"
              onClick={() => handleTheRequest("accept")}
            >
              Accept
            </button>
            <button
              className="btn-btn btn-reject"
              onClick={() => handleTheRequest("reject")}
            >
              Reject
            </button>
          </div>
